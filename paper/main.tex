...
We benchmark against classical FedAvg on MNIST ($n=10{,}000$) under non-IID
Dirichlet splits ($\alpha=0.5$) over 20 rounds, reporting accuracy,
communication cost, and privacy-utility tradeoff across six algorithm-privacy
configurations. Our results show that both FedAvg and q-FedAvg reach
100\% peak accuracy (no-DP, rounds to 90\%: 7), with graceful degradation
under strict privacy ($\varepsilon=1.0$: 75.0\% for FedAvg, 76.2\% for
q-FedAvg). Communication overhead is 2.44~MB per round for a
101,770-parameter MLP across 3~clients. The complete framework — including
Kubernetes deployment, GitHub Actions CI/CD with 100\% test coverage, and EU
AI Act compliance engine — is released as open-source software.
\end{abstract}
...
(strong privacy), QFL achieves 75.0\% accuracy, within 25.0 percentage points
of the no-DP baseline (100\%) across 20 rounds. Notably, q-FedAvg slightly
outperforms FedAvg under privacy constraints (76.2\% vs.\ 75.0\% at
$\varepsilon=1.0$), consistent with its fairness-aware aggregation reducing
the impact of high-noise clients.
